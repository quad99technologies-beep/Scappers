...
MAX_WORKERS = getenv_int("SCRIPT_01_NUM_WORKERS", 3)
MAX_RETRIES = getenv_int("SCRIPT_01_MAX_RETRIES_PER_PAGE", 5)
SLEEP_BETWEEN = getenv_float("SCRIPT_01_SLEEP_BETWEEN", 2.0)
REQUEST_TIMEOUT = getenv_float("SCRIPT_01_REQUEST_TIMEOUT", 120.0)
...
HEADERS = {
    ...
    "Accept-Language": "en-IN,en;q=0.9,kn;q=0.8,hi;q=0.7,kk;q=0.6",
...
# --------------------------------------------------
async def initialize_session(client: httpx.AsyncClient) -> tuple[bool, str]:
    """
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(f"[INIT] Establishing session (attempt {attempt}/{max_retries})...", flush=True)
            ...
            if attempt < max_retries:
                wait_time = 5 * attempt
                print(f"[INIT] Retrying in {wait_time}s...", flush=True)
                await asyncio.sleep(wait_time)
            else:
                print(f"[WARN] Failed to initialize session after {max_retries} attempts", flush=True)
...
async def fetch_page(
    ...
    """Fetch a single pager page and extract detail URLs using POST.
    ...
    # POST data for the pager request
    post_data = {
        "t:ac": "overview",
        "t:zoneid": zone_id,
    }
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            async with semaphore:
                # Use POST instead of GET (as the website does)
            ...
            if attempt < MAX_RETRIES:
                wait_time = 5 * attempt
                print(f"  [RETRY] Page {page_num}: 0 URLs, waiting {wait_time}s...", flush=True)
                await asyncio.sleep(wait_time)
                continue
            ...
            if attempt < MAX_RETRIES:
                ...
                print(f"  [RETRY] Page {page_num}: {error_type}, waiting {wait_time}s...", flush=True)
                await asyncio.sleep(wait_time)
...
async def collect_urls_async() -> None:
    ...
    if TOR_AVAILABLE:
        try:
            tor_cfg = TorConfig.from_env(getenv)
            tor_proxy_url = setup_tor(tor_cfg)
            if tor_proxy_url:
                print(f"[TOR] Using proxy: {tor_proxy_url}", flush=True)
            else:
        ...
        print("[TOR] core.tor_httpx not available — running without Tor", flush=True)
    ...
    async with httpx.AsyncClient(
        ...
        session_initialized, zone_id = await initialize_session(client)
        ...
        tasks = [fetch_page(client, p, semaphore, zone_id) for p in pages_to_process]
    ...
    try:
        run_id = os.environ.get("NORTH_MACEDONIA_RUN_ID", "")
        print(f"[DB] Using run_id: {run_id}", flush=True)
        if run_id:
