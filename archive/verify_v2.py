#!/usr/bin/env python3
"""Verify final output v2 matches expected data."""

import pandas as pd

expected = pd.read_csv('C:/Users/Vishw/OneDrive/Desktop/New folder (2)/FinalData.csv', encoding='latin-1')
extracted = pd.read_csv('output/Tender_Chile/final_tender_data_v2.csv', encoding='utf-8-sig')

print(f'Expected rows: {len(expected)}')
print(f'Extracted rows: {len(extracted)}')
print()

# Sort both by the same keys for comparison
