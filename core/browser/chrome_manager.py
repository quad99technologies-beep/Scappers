...
def kill_orphaned_chrome_processes(include_firefox: bool = False):
    """
    Kill orphaned Chrome/Chromium/Geckodriver processes that are not properly tracked.
    
    This is a nuclear cleanup option for when drivers crash or are not properly closed.
    Use with caution on shared systems.
    
    Args:
        include_firefox: If True, also kill Firefox and geckodriver processes
    ...
    try:
        ...
        logger.warning("[CLEANUP] psutil not available, cannot kill orphaned processes")
        return 0
    ...
    if include_firefox:
        process_names.extend(["firefox", "geckodriver"])
    
    logger.info(f"[CLEANUP] Searching for orphaned browser processes: {', '.join(process_names)}")
    ...
    for proc in psutil.process_iter(['pid', 'name']):
        try:
            ...
            if any(target in proc_name for target in process_names):
                try:
                    p = psutil.Process(proc.info['pid'])
                    p.kill()
                    killed_count += 1
                    logger.info(f"[CLEANUP] Killed {proc_name} PID {proc.info['pid']}")
                except (psutil.NoSuchProcess, psutil.AccessDenied) as e:
                    logger.debug(f"[CLEANUP] Could not kill {proc_name} PID {proc.info['pid']}: {e}")
        except (psutil.NoSuchProcess, psutil.AccessDenied):
            continue
    
    if killed_count > 0:
        logger.warning(f"[CLEANUP] Killed {killed_count} orphaned browser process(es)")
        # Give processes time to terminate
        import time
        time.sleep(2)
    else:
        logger.debug("[CLEANUP] No orphaned browser processes found")
