...
def norm_key_alphanumeric(s: Any) -> str:
    """Normalize a value for use in a composite key: strip, lower, remove all non-alphanumeric."""
...
def build_product_key(
    ...
    """
    Build a composite key from run_id, registration_no, product_name, generic_name, dosage_form, strength.
    All special characters are removed; only alphanumeric characters are kept (lowercase).
    Use for dedup, lookups, or UNIQUE-style keys.
    """
    return (
        norm_key_alphanumeric(run_id)
        + norm_key_alphanumeric(registration_no)
        + norm_key_alphanumeric(product_name)
        + norm_key_alphanumeric(generic_name)
        + norm_key_alphanumeric(dosage_form)
        + norm_key_alphanumeric(strength)
    )
