...
FIXES APPLIED:
1) ✅ START_PAGE_1IDX = 1 (was 6) - Now starts from page 1, no data loss
2) ✅ MAX_ROWS configurable via env var, default None (was 500) - No artificial limit
3) ✅ Integrated encoding utilities for UTF-8/mojibake fixes
4) ✅ Improved error handling and logging
5) ✅ Consistent BASE_DIR path handling
6) ✅ Better parsing logic with validation
...
if not INPUT_PDF.exists():
    ...
    alt_paths = [
        BASE_DIR / "input" / "annexe_v.pdf",
        Path(__file__).resolve().parent / "annexe_v.pdf",
    ]
...
OUTPUT_CSV = OUTPUT_DIR / "annexe_v_extracted.csv"
LOG_FILE = OUTPUT_DIR / "annexe_v_extraction_log.txt"

# Page range - FIXED: Start from page 1 (not page 6!)
START_PAGE_1IDX = 1  # ✅ FIXED: Was 6, now 1 to capture all data

# Row limit - FIXED: Configurable via env var, default None (no limit)
...
def extract_annexe_v():
            ...
            with csv_writer_utf8(OUTPUT_CSV, add_bom=True) as f:
                ...
                for page_no_1idx in range(START_PAGE_1IDX, total_pages + 1):
                    try:
                                ...
                                if din_idx is not None:
                                    ...
                                    for k in range(n):
                                        ...
                                        if MAX_ROWS is not None and total_rows >= MAX_ROWS:
                                            ...
                                            print(f"\n⚠️  MAX_ROWS reached ({MAX_ROWS}). Stopping early for test.")
                        ...
                        print(f"⚠️  Error on page {page_no_1idx}: {e}")
                        continue
    ...
    if errors_encountered > 0:
        print(f"⚠️  Errors encountered: {errors_encountered}")
    
    # Post-processing: Remove rows with footer values in price column
    print(f"\n🧹 Cleaning footer values from price column...")
    cleaned_count = clean_footer_values_from_csv(OUTPUT_CSV, logger)
    print(f"✅ Removed {cleaned_count} rows with footer values (dates/identifiers)")
