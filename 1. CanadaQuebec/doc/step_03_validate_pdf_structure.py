...
def write_reports(result: Dict[str, Any]) -> None:
    ...
    # JSON report
    json_path = OUTPUT_DIR / "pdf_structure_report.json"
    ...
    txt_lines = [
        f"Input: {s['input']}",
        f"Pages total: {s['pages_total']}",
        f"Pages with DIN: {s['pages_with_din']}",
        f"Legend detected: {s['legend_detected']}",
        f"Headers over DIN pages: {s['ratios']['headers_over_din_pages']:.0%}",
        f"Row shape OK: {s['ratios']['rowshape_over_din_pages']:.0%}",
        f"Flagged pages: {s['flagged_pages_count']} ({s['ratios']['flagged_over_din_pages']:.0%})",
        "",
        f"FIT TO RUN: {'YES' if s['fit_to_run'] else 'NO'}"
    ]
    if s["fit_reasons"]:
        txt_lines.append("Notes:")
        txt_lines.extend(f"  - {r}" for r in s["fit_reasons"])
    txt_lines.append("")
    txt_lines.append("Flagged pages (top 50):")
    for fp in result["flagged"][:50]:
        issues = "; ".join(fp["issues"])
        txt_lines.append(f"  - p.{fp['page']}: {issues}")

    txt_path = OUTPUT_DIR / "pdf_structure_report.txt"
    with open(txt_path, "w", encoding="utf-8", errors='replace') as f:
        f.write("\n".join(txt_lines))

    # CSV report
    csv_path = OUTPUT_DIR / "pdf_structure_flags.csv"
    with open(csv_path, "w", newline="", encoding="utf-8", errors='replace') as f:
        writer = csv.DictWriter(f, fieldnames=[
            "page", "din_rows", "ok_rows", "unit_only_lines", 
            "pack_continuations", "generics_detected", "headers_found", "issues"
        ])
        writer.writeheader()
        for fp in result["flagged"]:
            writer.writerow({
                ...
                "ok_rows": fp["ok_rows"],
                "unit_only_lines": fp["unit_only_lines"],
                "pack_continuations": fp["pack_continuations"],
                "generics_detected": fp["generics_detected"],
                "headers_found": fp["headers_found"],
                "issues": " | ".join(fp["issues"])
            })
...
def main() -> None:
    ...
    pdf_path = (INPUT_PDF if INPUT_PDF.exists() 
                else next(SPLIT_PDF_DIR.glob("*.pdf"), None))
    if not pdf_path:
        pdf_path = next(INPUT_DIR.glob("*.pdf"), None)
    if not pdf_path:
        print(json.dumps({"status": "error", 
                         "message": f"No PDF found in {SPLIT_PDF_DIR} or {INPUT_DIR}"}))
        return
