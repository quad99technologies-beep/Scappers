...
# Folders
BASE_DIR   = Path(__file__).resolve().parent.parent  # Go up from script/ to root
REPORT_DIR = BASE_DIR / "Output"
OUTPUT_DIR = BASE_DIR / "Output"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Find latest alfabeta_Report_*.xlsx
candidates = sorted(REPORT_DIR.glob("alfabeta_Report_*.xlsx"),
                    key=lambda p: p.stat().st_mtime,
                    reverse=True)
if not candidates:
    raise FileNotFoundError(f"No alfabeta_Report_*.xlsx found in {REPORT_DIR}. Run script 05 first to generate the report.")
...
# Save
out_path = OUTPUT_DIR / "pcid_MISSING.xlsx"
