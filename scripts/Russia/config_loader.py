"""
Configuration Loader for Russia Scraper (Platform Config Integration)

This module wraps platform_config.py for centralized path management.
All configuration is loaded from config/Russia.env.json.

Precedence (highest to lowest):
1. Runtime overrides
2. Environment variables (OS-level)
3. Platform config (config/Russia.env.json)
...
# Now: scripts/Russia/config_loader.py -> parents[2] = repo root
...
def get_central_output_dir() -> Path:
    """Get central exports directory for final reports - uses Documents/ScraperPlatform/output/exports/Russia/"""
...
def load_env_file():
    """
    Load environment variables from platform.env and Russia.env.
...
def require_env(key: str) -> str:
    ...
    if value is None or value == "":
        raise ValueError(f"Required environment variable '{key}' is not set in config file. Please add it to config/Russia.env.json")
    return value
...
def get_base_dir() -> Path:
    """
    Get base directory for Russia scraper.
...
def get_input_dir(subpath: str = None) -> Path:
    """
    Get input directory - uses Documents/ScraperPlatform/input/Russia/
...
def get_output_dir(subpath: str = None) -> Path:
    """
    Get output directory - uses Documents/ScraperPlatform/output/Russia/
...
def get_backup_dir() -> Path:
    """Get backup directory - scraper-specific: backups/Russia/"""
    if _PLATFORM_CONFIG_AVAILABLE:
        pm = get_path_manager()
...
if __name__ == "__main__":
    print("=" * 60)
    print("Russia Config Loader - Diagnostic")
    print("=" * 60)
    ...
    print(f"  SCRIPT_01_HEADLESS: {getenv('SCRIPT_01_HEADLESS', 'not set')}")
    print(f"  SCRIPT_01_REGION_VALUE: {getenv('SCRIPT_01_REGION_VALUE', 'not set')}")
