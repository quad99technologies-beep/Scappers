...
def process_files(ved_input: Path, excluded_input: Path,
    ...
    # Collect ALL unique Cyrillic values from both files
    all_unique = set()
    all_unique.update(collect_unique_values(ved_rows, TRANSLATE_COLUMNS))
    all_unique.update(collect_unique_values(excluded_rows, TRANSLATE_COLUMNS))
