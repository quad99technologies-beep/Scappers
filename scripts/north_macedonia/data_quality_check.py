#!/usr/bin/env python3
"""North Macedonia scraper data-quality guard."""
...
def _get_db():
    """Get a DB connection for North Macedonia."""
    try:
        from core.db.postgres_connection import get_db
        return get_db("NorthMacedonia")
...
def _get_run_id(output_dir: Path) -> Optional[str]:
    run_id_env = os.environ.get("NORTH_MACEDONIA_RUN_ID")
    if run_id_env:
...
def check_db_table_counts(db, run_id: Optional[str]) -> List[CheckResult]:
    """Check row counts for all nm_ tables."""
    results: List[CheckResult] = []
    if db is None:
        return [CheckResult("DB", "Connection", FAIL, "No database connection")]

    tables = {
        "nm_urls": ("Collected URLs", 100),
        "nm_drug_register": ("Drug register entries", 50),
        "nm_pcid_mappings": ("PCID mappings", 0),
        "nm_final_output": ("Final output rows", 0),
        "nm_input_dictionary": ("Dictionary entries", 50),
        "nm_errors": ("Error log", None),
        "nm_statistics": ("Statistics", None),
        "nm_validation_results": ("Validation results", None),
    }

    for table, (label, min_rows) in tables.items():
        try:
            if run_id and table not in ("nm_input_dictionary",):
...
def check_null_key_fields(db, run_id: Optional[str]) -> List[CheckResult]:
    ...
    checks = [
        ("nm_urls", "detail_url", "Detail URL"),
        ("nm_drug_register", "local_product_name", "Product name"),
        ("nm_drug_register", "local_pack_code", "Pack code"),
        ("nm_drug_register", "generic_name", "Generic name"),
    ]
...
def check_duplicates(db, run_id: Optional[str]) -> List[CheckResult]:
    ...
    try:
        ...
        row = _q1(db, f"SELECT COUNT(*) - COUNT(DISTINCT detail_url) FROM nm_urls {where}", params)
        dups = row[0] if row else 0
        status = PASS if dups == 0 else WARN
        results.append(CheckResult("Duplicates", "URL uniqueness", status, f"{dups} duplicate URLs"))
    except Exception as exc:
        results.append(CheckResult("Duplicates", "URL uniqueness", WARN, f"Check failed: {exc}"))

    # Duplicate pack codes
    try:
        where = "WHERE run_id = %s" if run_id else ""
        params = (run_id,) if run_id else ()
        row = _q1(db, f"SELECT COUNT(*) - COUNT(DISTINCT local_pack_code) FROM nm_drug_register {where}", params)
        dups = row[0] if row else 0
        status = PASS if dups == 0 else WARN
        results.append(CheckResult("Duplicates", "Pack code uniqueness", status, f"{dups} duplicate codes"))
    except Exception as exc:
        results.append(CheckResult("Duplicates", "Pack code uniqueness", WARN, f"Check failed: {exc}"))

    # Duplicate dictionary terms
    try:
        row = _q1(db, "SELECT COUNT(*) - COUNT(DISTINCT source_term) FROM nm_input_dictionary")
        dups = row[0] if row else 0
        status = PASS if dups == 0 else WARN
        results.append(CheckResult("Duplicates", "Dictionary term uniqueness", status, f"{dups} duplicate terms"))
    except Exception as exc:
...
def check_scrape_coverage(db, run_id: Optional[str]) -> List[CheckResult]:
    """Check how many URLs have been successfully scraped."""
    results: List[CheckResult] = []
    if db is None or not run_id:
        return results

    try:
        total_row = _q1(db, "SELECT COUNT(*) FROM nm_urls WHERE run_id = %s", (run_id,))
        scraped_row = _q1(db, "SELECT COUNT(*) FROM nm_urls WHERE run_id = %s AND status = 'scraped'", (run_id,))
        total = total_row[0] if total_row else 0
        scraped = scraped_row[0] if scraped_row else 0
        if total > 0:
            pct = scraped / total * 100
            status = PASS if pct >= 95 else (WARN if pct >= 80 else FAIL)
            detail = f"{scraped}/{total} URLs scraped ({pct:.1f}%)"
        else:
            status = WARN
            detail = "No URLs collected"
        results.append(CheckResult("Coverage", "URL scrape coverage", status, detail))
    except Exception as exc:
        results.append(CheckResult("Coverage", "URL scrape coverage", WARN, f"Check failed: {exc}"))

    return results


def check_referential_integrity(db, run_id: Optional[str]) -> List[CheckResult]:
    ...
    try:
        row = _q1(db,
            "SELECT COUNT(*) FROM nm_drug_register dr "
            "WHERE dr.run_id = %s AND dr.url_id IS NOT NULL "
            "AND NOT EXISTS (SELECT 1 FROM nm_urls u WHERE u.id = dr.url_id)",
            (run_id,),
        )
        orphans = row[0] if row else 0
        status = PASS if orphans == 0 else WARN
        results.append(CheckResult("Integrity", "Drug register -> URLs FK", status, f"{orphans} orphan rows"))
    except Exception as exc:
        results.append(CheckResult("Integrity", "Drug register -> URLs FK", WARN, f"Check failed: {exc}"))
...
def check_export_files() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    exports_dir = REPO_ROOT / "exports" / "NorthMacedonia"

    for suffix in ("pcid_mapped", "pcid_missing", "pcid_no_data"):
...
def persist_report(results: List[CheckResult], base_dir: Path):
    ...
    txt_path = reports_dir / f"north_macedonia_data_quality_{ts}.txt"
    json_path = reports_dir / f"north_macedonia_data_quality_{ts}.json"
    with txt_path.open("w", encoding="utf-8") as fh:
        fh.write("North Macedonia Data Quality Report\n")
        fh.write("=" * 80 + "\n")
...
def main() -> None:
    ...
    results.extend(check_referential_integrity(db, run_id))
