#!/usr/bin/env python3
"""
North Macedonia Scraper - Enterprise Edition
...
class NorthMacedoniaScraper(BaseScraper):
    def __init__(self):
        super().__init__("NorthMacedonia")
        
    def run(self):
        self.logger.info("Starting North Macedonia Enterprise Pipeline")
        
        steps = [
            ("00_backup_and_clean.py", "Backup and Clean", True),
            ("01_collect_urls.py", "Collect URLs", True),
            ("02_fast_scrape_details.py", "Scrape Details", True),
            ("04_translate_using_dictionary.py", "Translate", True),
            ("05_stats_and_validation.py", "Validation", False), # Non-blocker
            ("06_generate_export.py", "Generate Export", True),
        ]
        ...
        for script, desc, strict in steps:
            ...
            try:
                env = os.environ.copy()
                env["NORTHMACEDONIA_RUN_ID"] = self.run_id
        ...
        self.logger.info("North Macedonia Pipeline Completed successfully")

if __name__ == "__main__":
    try:
        scraper = NorthMacedoniaScraper()
        scraper.run()
