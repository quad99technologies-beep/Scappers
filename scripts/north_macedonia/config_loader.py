"""
Configuration Loader for North Macedonia Scraper (Platform Config Integration)

This module wraps platform_config.py for centralized path management.
All configuration is loaded from config/NorthMacedonia.env.json.

Precedence (highest to lowest):
1. Runtime overrides
2. Environment variables (OS-level)
3. Platform config (config/NorthMacedonia.env.json)
...
# scripts/North Macedonia/config_loader.py -> parents[2] = repo root
...
def get_central_output_dir() -> Path:
    """Get central exports directory for final reports - exports/NorthMacedonia/."""
...
def load_env_file():
    """
    Load environment variables from platform.env and NorthMacedonia.env.
...
def get_base_dir() -> Path:
    """Get base directory for North Macedonia scraper."""
    if _PLATFORM_CONFIG_AVAILABLE:
        # Migrated: get_path_manager() -> ConfigManager
...
def get_input_dir(subpath: str = None) -> Path:
    """Get input directory - input/NorthMacedonia/."""
    if _PLATFORM_CONFIG_AVAILABLE:
        # Migrated: get_path_manager() -> ConfigManager
...
def get_output_dir(subpath: str = None) -> Path:
    """Get output directory - output/NorthMacedonia/."""
...
def get_backup_dir() -> Path:
    """Get backup directory - backups/NorthMacedonia/."""
    if _PLATFORM_CONFIG_AVAILABLE:
        # Migrated: get_path_manager() -> ConfigManager
...
if __name__ == "__main__":
    print("=" * 60)
    print("North Macedonia Config Loader - Diagnostic")
    print("=" * 60)
