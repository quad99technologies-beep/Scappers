...
try:
    from core.browser.stealth_profile import apply_playwright
...
try:
    from core.browser.browser_observer import observe_playwright, wait_until_idle
...
# CSV/JSON paths - only used for final export (all data goes to DB during scraping)
...
_repo: Optional['NetherlandsRepository'] = None
...
# -------------------------
# Data helpers (DB-first)
# -------------------------

def get_scraped_detail_urls() -> Set[str]:
    """Get already-scraped detail URLs from DB."""
    global _repo
    if _repo is not None:
        return _repo.get_scraped_detail_urls()
    return set()


def insert_detail(detail: Dict) -> None:
    """Insert a detail record to DB."""
    global _repo
    if _repo is not None:
        _repo.insert_details([detail])


def insert_costs(costs: List[Dict], detail_url: str = None) -> None:
    """Insert cost records to DB."""
    global _repo
    if _repo is not None:
        _repo.insert_costs(costs, detail_url)


def mark_detail_scraped(url: str) -> None:
    """Mark a detail URL as scraped in step progress."""
    global _repo, _run_id
    if _repo is not None:
        _repo.mark_progress(
            step_number=2,
            step_name="reimbursement_extraction",
            progress_key=url,
            status="completed"
        )


def log_error(error_type: str, message: str, url: str = None,
              thread_id: int = None, context: Dict = None) -> None:
    """Log an error to DB."""
    global _repo
    if _repo is not None:
        _repo.log_error(
...
# -------------------------
# Legacy persistence helpers (for CSV export only)
# -------------------------
...
def append_rows(path: Path, fieldnames: List[str], rows: List[Dict], lock: threading.Lock) -> None:
    """Append rows to CSV file (legacy, for export only)."""
    if not rows:
        return
    with lock:
...
def phase1_collect_urls() -> List[str]:
    ...
    with sync_playwright() as p:
        ...
        # Track Playwright browser PIDs using standardized ChromeInstanceTracker
        try:
            ...
            if browser_pids:
                # Use standardized ChromeInstanceTracker
                try:
                    run_id = os.environ.get("NETHERLANDS_RUN_ID") or ""
                    if not run_id:
                        # Try to get from file or generate
                        run_id_file = Path(__file__).parent.parent / "output" / ".current_run_id"
                        if run_id_file.exists():
                            run_id = run_id_file.read_text(encoding="utf-8").strip()
                    
                    if run_id:
                        db = CountryDB("Netherlands")
                        tracker = ChromeInstanceTracker(scraper_name, run_id, db)
                        for pid in browser_pids:
                            tracker.register(
                                step_number=2,
                                thread_id=0,  # Phase 1 is single-threaded
                                pid=pid,
                                browser_type="chrome",
                    ...
                    pass  # DB registration not critical
        except Exception:
            pass  # PID tracking not critical

        context_kwargs = {}

        # Apply enhanced standardized stealth profile
        if STEALTH_PROFILE_AVAILABLE:
            apply_playwright(context_kwargs)
            # Also inject stealth init script
            try:
                from core.browser.stealth_profile import get_stealth_init_script, apply_playwright_with_script
                # apply_playwright already called above, but we need to inject script later
            except ImportError:
                pass

        ctx = browser.new_context(**context_kwargs) if context_kwargs else browser.new_context()
        
        # Inject standardized stealth init script
        try:
        ...
        # Save URLs to JSON file for backup/debugging (optional)
        try:
            with open(ALL_URLS_JSON, 'w', encoding='utf-8') as f:
                json.dump(detail_urls, f, indent=2, ensure_ascii=False)
            print(f"[SAVE]   Saved {len(detail_urls)} URLs to {ALL_URLS_JSON}")
        except Exception as e:
            print(f"[WARNING] Failed to save URLs to JSON: {e}")
...
def worker_run(
    ...
    def create_browser_and_page(p):
        ...
        try:
            ...
            geo_config = get_geo_config_for_scraper("Netherlands")
            
            if geo_config:
                # Apply geo-specific settings
                context_kwargs["timezone_id"] = geo_config.get("timezone", "Europe/Amsterdam")
                context_kwargs["locale"] = geo_config.get("locale", "nl-NL")
                
                if geo_config.get("geolocation"):
                    context_kwargs["geolocation"] = geo_config["geolocation"]
                ...
                if geo_config.get("proxy"):
                    ...
                    context_kwargs["proxy"] = {
                        "server": f"{proxy['protocol']}://{proxy_server}",
                        "username": proxy.get("username"),
                        "password": proxy.get("password")
                    }
                    logger.info(f"[GEO_ROUTER] Using proxy for Netherlands: {proxy['host']}:{proxy['port']}")
        except Exception as e:
            logger.debug(f"[GEO_ROUTER] Integration failed (non-fatal): {e}")
    ...
    with sync_playwright() as p:
        ...
        while True:
            ...
            try:
                ...
                # Write to DB (replaces CSV append)
                detail_record = {
                    "detail_url": url,
                ...
                insert_detail(detail_record)

                if costs:
                    cost_rows = [{"detail_url": url, **c} for c in costs]
                    insert_costs(cost_rows, detail_url=url)
                ...
                # Mark scraped in DB and local set
                mark_detail_scraped(url)
                with scraped_lock:
                    scraped_set.add(url)
...
def phase2_extract_details_mt(detail_urls: List[str]) -> None:
    ...
    # Resume state from DB
    scraped = get_scraped_detail_urls()
    print(f"[DB] Loaded {len(scraped)} already-scraped URLs from database")
...
def main(run_id: str = None):
    """
    Main entry point for Step 2 (farmacotherapeutischkompas.nl scraping).

    Args:
        run_id: Optional run ID. If not provided, tries environment variable, then generates one from timestamp.
    """
    global _repo, _run_id
    ...
    print("=" * 60)
    print("NETHERLANDS STEP 2: FARMACOTHERAPEUTISCHKOMPAS.NL SCRAPER")
    ...
    if DB_AVAILABLE:
        try:
            ...
            _repo = NetherlandsRepository(db, run_id)
            ...
            print("[DB] Continuing without database (data will not be persisted)")
            _repo = None
    else:
        print("[DB] WARNING: Database module not available")
        _repo = None

    # Ensure CSV headers for legacy/export
    ...
    # Export CSVs from DB (for backward compatibility)
    if _repo is not None:
        print("\n" + "=" * 60)
        print("EXPORTING DATA TO CSV FILES")
        print("=" * 60)
        try:
            details_count = _repo.export_details_csv(DETAILS_CSV)
            costs_count = _repo.export_costs_csv(COSTS_CSV)
            print(f"[EXPORT] details.csv: {details_count} rows")
            print(f"[EXPORT] costs.csv: {costs_count} rows")
        except Exception as e:
            print(f"[EXPORT] WARNING: CSV export failed: {e}")
    ...
    # Print final stats
    if _repo is not None:
        stats = _repo.get_run_stats()
        print(f"[STATS] Details scraped: {stats.get('details_total', 0)}")
        print(f"[STATS] Cost entries: {stats.get('costs_total', 0)}")
        print(f"[STATS] Errors: {stats.get('error_count', 0)}")
