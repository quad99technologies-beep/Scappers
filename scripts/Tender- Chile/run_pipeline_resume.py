...
def run_step(step_num: int, script_name: str, step_name: str, total_steps: int, output_files: list = None):
    ...
    try:
        terminate_scraper_pids("Tender_Chile", _repo_root, silent=True)
        subprocess.run([sys.executable, "-u", str(script_path)], check=True, capture_output=False)
        duration_seconds = time.time() - start_time

        cp = get_checkpoint_manager("Tender_Chile")
        if output_files:
...
def main():
    ...
    output_dir = get_output_dir()
    tender_list = getenv("SCRIPT_01_OUTPUT_CSV", "tender_list.csv")
    redirect_csv = getenv("SCRIPT_02_OUTPUT_CSV", "tender_redirect_urls.csv")
    details_csv = getenv("SCRIPT_03_OUTPUT_CSV", "tender_details.csv")
    supplier_csv = getenv("SCRIPT_04_SUPPLIER_OUTPUT_CSV", "mercadopublico_supplier_rows.csv")
    lot_csv = getenv("SCRIPT_04_LOT_OUTPUT_CSV", "mercadopublico_lot_summary.csv")
    final_csv = getenv("SCRIPT_05_FINAL_OUTPUT_CSV", "final_tender_data.csv")

    steps = [
        (0, "00_backup_and_clean.py", "Backup and Clean", None),
        (1, "1. GetTender.py", "Load Tender List", [str(output_dir / tender_list)]),
        (2, "2. Get Redirect URLs.py", "Build Details URLs", [str(output_dir / redirect_csv)]),
        (3, "3. Chillie Tender.py", "Extract Tender Details", [str(output_dir / details_csv)]),
        (4, "4. Chillie Tender Award.py", "Extract Award Data", [str(output_dir / supplier_csv), str(output_dir / lot_csv)]),
        (5, "5. Merge Final CSV.py", "Merge Final CSV", [str(output_dir / final_csv)]),
    ]
    ...
    for step_num, script_name, step_name, output_files in steps:
        if step_num < start_step:
            if cp.should_skip_step(step_num, step_name, verify_outputs=True, expected_output_files=output_files):
                ...
                if completion_percent > 100.0:
                    completion_percent = 100.0
                print(f"[PROGRESS] Pipeline Step: {step_num + 1}/{total_steps} ({completion_percent}%) - Skipped: Output already generated", flush=True)
            continue
