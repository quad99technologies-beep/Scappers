...
# -*- coding: utf-8 -*-
"""
Progress UI Module for Tender Chile Scraper
...
class ChileProgressUI:
    """Rich progress UI for Tender Chile scraper with checkpoint visualization."""
    ...
    def show_pipeline_start(self):
        ...
        console.rule(f"[bold blue]Tender Chile Scraper - Run: {self.run_id}")
    ...
    def create_step_progress(self, task_name: str, total: int) -> ScraperProgress:
        ...
        return ScraperProgress(f"tc_{task_name}")
...
class ChileCheckpointVisualizer:
    """Visualize checkpoint and resume state for Tender Chile."""
    ...
    def render_checkpoint_flow(self):
        ...
        for i in range(5):
            ...
            name = ["Backup", "URLs", "Details", "Awards", "Merge"][i]
...
def create_standard_progress(task_id: str, total: int, run_dir: Path = None) -> StandardProgress:
    """Create a StandardProgress tracker for Tender Chile."""
    state_path = None
    if run_dir:
...
if __name__ == "__main__":
    ...
    ui = ChileProgressUI("demo_run_001")
    ...
    checkpoint_info = {
        "completed_steps": [0, 1],
        "step_data": {
            "0": {"duration_seconds": 3.5, "output_files": []},
            "1": {"duration_seconds": 25.2, "output_files": ["redirects.csv"]},
        }
    }
