"""
Shared utilities for Tender Chile scraper scripts.
...
def force_cleanup():
    ...
    gc.collect()
    # Clear any unreferenced objects
    for _ in range(3):
        gc.collect()
...
def cleanup_orphaned_chrome_processes():
    """Kill any orphaned Chrome processes not in active drivers list"""
    killed_count = 0
    current_pids = set()
...
def log_progress_with_step(step: str, completed: int = None, total: int = None):
    ...
    global _tenders_completed, _total_tenders, _current_step
    
    with _progress_lock:
        if completed is None:
            completed = _tenders_completed
        if total is None:
            total = _total_tenders
        _current_step = step
...
def update_progress(completed: int, total: int = None):
    ...
    with _progress_lock:
        ...
        if total is not None:
            _total_tenders = total


def set_total_tenders(total: int):
    """Set total tenders count"""
