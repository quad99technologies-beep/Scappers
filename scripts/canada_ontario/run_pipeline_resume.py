...
def _refresh_run_id():
    """Re-read run_id from .current_run_id after step 0 generates a new one.

    Step 0 (backup_and_clean) generates a fresh run_id and writes it to
    .current_run_id. The pipeline runner must pick up this new run_id so
    subsequent steps use the correct value (avoids FK constraint errors).
    """
    ...
    if not run_id_file.exists():
        return
    new_run_id = run_id_file.read_text(encoding="utf-8").strip()
    if not new_run_id or new_run_id == RUN_ID:
        return
    old_run_id = RUN_ID
    RUN_ID = new_run_id
    RUN_DIR = get_run_dir(RUN_ID)
    os.environ["CANADA_ONTARIO_RUN_ID"] = new_run_id
    os.environ["RUN_ID"] = new_run_id
    logger.info("Run ID refreshed after step 0: %s -> %s", old_run_id, new_run_id)
...
def run_step(step_meta: Dict, progress: StandardProgress):
    ...
    try:
        ...
        logger.info(f"OK: Step {step_num} ({step_name}) completed in {duration:.2f}s")
...
def main():
    ...
    # Determine if we are in DB-only mode
    db_only = getenv_bool("DB_ONLY", True)
    ...
    try:
        for step in steps:
            ...
            # After step 0, pick up the new run_id generated by backup_and_clean
            if step["id"] == 0:
                _refresh_run_id()
    ...
    logger.info("All data migrated to PostgreSQL database successfully")
    logger.info("-" * 50)
    try:
