#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Delete products with no data (total_records=0)"""
...
# Count products with no data
with db.cursor() as cur:
    cur.execute("""
        SELECT COUNT(*) FROM ar_product_index
        WHERE run_id = %s AND COALESCE(total_records, 0) = 0
    """, (run_id,))
    count = cur.fetchone()[0]

print(f"Found {count} products with total_records=0 (no data)")
...
if count > 0:
    ...
    with db.cursor() as cur:
        cur.execute("""
            ...
            WHERE run_id = %s AND COALESCE(total_records, 0) = 0
        """, (run_id,))
    print(f"[OK] Deleted {count} products with no data from ar_product_index")

    # Verify
    with db.cursor() as cur:
        cur.execute("SELECT COUNT(*) FROM ar_product_index WHERE run_id = %s", (run_id,))
        remaining = cur.fetchone()[0]
    print(f"[OK] Remaining products in index: {remaining}")
else:
