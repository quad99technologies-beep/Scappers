"""
State Machine Module for Argentina Scraper
...
class NavigationStateMachine:
    ...
    def _check_loading_indicator(self, page_or_driver: Any) -> bool:
        ...
        try:
            if hasattr(page_or_driver, 'inner_text'):  # Playwright
                ...
                body_text = page_or_driver.find_element_by_tag_name("body").text.lower()
                return "loading" in body_text or "please wait" in body_text
        except Exception:
            return False
    ...
    def _check_table_has_data(self, page_or_driver: Any) -> bool:
        ...
        try:
            if hasattr(page_or_driver, 'query_selector_all'):  # Playwright
                ...
                rows = page_or_driver.find_elements_by_css_selector("table tbody tr, table tr")
    ...
    def validate_state(self, target_state: NavigationState,
        ...
        self.logger.info(f"[STATE] Validating state: {target_state.value} - {state_def.description}")
        ...
        self.logger.info(f"[STATE] State validated: {target_state.value}")
        return True
