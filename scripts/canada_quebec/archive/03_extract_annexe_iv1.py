...
def generic_base(generic: str) -> str:
    ...
    if not generic:
        return ""
    g = str(generic).strip()
    g = re.sub(r"\s+[X]\s*$", "", g)  # Remove trailing " X"
...
def extract_strength(desc: str):
    ...
    m = re.search(r"(\d+(?:[.,]\d+)?)\s*(mg|mcg|g|u|ui|iu)\s*/\s*(\d+(?:[.,]\d+)?)\s*mL", s, flags=re.IGNORECASE)
    if m:
        val = float(m.group(1).replace(",", "."))
        unit = m.group(2).lower()
        return (int(val) if val.is_integer() else val), unit.upper()

    # 2) Concentration-style: "50 mg/mL (0.8 mL)"
    c = re.search(r"(\d+(?:[.,]\d+)?)\s*(mg|mcg|g|u|ui|iu)\s*/\s*mL", s, flags=re.IGNORECASE)
    v = re.search(r"\(\s*(\d+(?:[.,]\d+)?)\s*ml\s*\)", s, flags=re.IGNORECASE)
    if c and v:
        conc = float(c.group(1).replace(",", "."))
        unit = c.group(2).lower()
        vol = float(v.group(1).replace(",", "."))
        dose = conc * vol
        dose = int(dose) if float(dose).is_integer() else round(dose, 4)
        return dose, unit.upper()

    # 3) Simple dose fallback: "100 mg", "100 U"
    m = re.search(r"(\d+(?:[.,]\d+)?)\s*(mg|mcg|g|u|ui|iu)\b", s, flags=re.IGNORECASE)
    if m:
        val = float(m.group(1).replace(",", "."))
        unit = m.group(2).lower()
        return (int(val) if val.is_integer() else val), unit.upper()
