...
import pdfplumber
from pypdf import PdfReader  # Hybrid fallback
from openai import OpenAI    # Hybrid fallback
...
# AI Configuration (Hybrid)
from config_loader import OPENAI_API_KEY, OPENAI_MODEL
...
if not INPUT_PDF.exists():
    # Check strict mode and fallback settings
    ...
    if CONF_STRICT_MODE:
        # Strict mode: fail if PDF not found in expected location
        raise FileNotFoundError(
            f"Input PDF not found in strict mode: {INPUT_PDF}\n"
            f"Please ensure Step 1 (Split PDF) completed successfully."
        )
    
    # Try alternative locations (only if fallbacks allowed)
    alt_paths = []
    
    # Try input directory fallback
    try:
        input_dir = get_input_dir()
        alt_paths.append(input_dir / ANNEXE_V_PDF_NAME)
    except Exception:
        # If config_loader fails, use env fallback or default
        if not CONF_STRICT_MODE:
    ...
    # Try script directory fallback (only if allowed)
...
# FINAL_COLS is now loaded from config_loader as FINAL_COLUMNS
...
# REGEX PATTERNS
# =========================
RE_DIN = re.compile(r"^\d{6,9}$")  # Allow 6-9 digits (was 6-8)
...
# =========================
def extract_page_with_ai(text_blob: str) -> List[Dict[str, Any]]:
    """
    Fallback: Use OpenAI to parse a messy text dump from pypdf.
    Only called if pdfplumber fails to find structure.
    """
    if not client:
        return []
    ...
    prompt = (
        "Extract tabular data from this extracted PDF text.\n"
        "Columns usually: Code/DIN, Brand, Manufacturer, Format, Price, Unit Price.\n"
        "Return JSON with 'rows'. Capture the Generic Name context if present above rows.\n"
        "DIN must be numeric (6-9 digits). Price and UnitPrice are numbers (convert French ',' to '.').\n"
    ...
    try:
        resp = client.responses.create(
            ...
            input=[{
                "role": "user", 
                "content": [
                    {"type": "input_text", "text": prompt + "\n\nTEXT:\n" + text_blob[:15000]} # Limit context
                ]
            }],
            text={"format": {"type": "json_schema", "name": "annexe_v_hybrid", "schema": schema, "strict": True}}
        )
        return json.loads(resp.output_text).get("rows", [])
...
def extract_annexe_v():
            ...
            with csv_writer_utf8(OUTPUT_CSV, add_bom=True) as f:
                ...
                for page_no_1idx in range(START_PAGE_1IDX, total_pages + 1):
                    try:
                        ...
                        if page_rows == 0:
                            # If pdfplumber found no rows, check if pypdf sees meaningful text
                            try:
                                reader = PdfReader(str(INPUT_PDF))
                                if page_no_1idx - 1 < len(reader.pages):
                                    raw_text = reader.pages[page_no_1idx - 1].extract_text()
                                    # If text is substantial (>100 chars) and contains digits (potential DIN/Price)
                                    if len(raw_text) > 100 and re.search(r"\d{6}", raw_text):
                                        logger.info(f"Page {page_no_1idx}: pdfplumber failed. Attempting AI Hybrid extraction...")
                                        print(f"  [HYBRID] Using AI for Page {page_no_1idx}...", flush=True)
                                        
                                        ai_rows = extract_page_with_ai(raw_text)
                                        for r in ai_rows:
                                            din_val = r.get("DIN", "").strip()
                                            if not din_val: continue
                                            
                                            row = {
                                                "Generic Name": r.get("Generic") or current_generic,
                                                "Currency": STATIC_CURRENCY,
                                                "Ex Factory Wholesale Price": r.get("Price"),
                                                "Unit Price": r.get("UnitPrice"),
                                                "Region": STATIC_REGION,
                                                "Product Group": r.get("Brand") or (r.get("Generic") if not current_generic else ""),
                                                "Marketing Authority": r.get("Manufacturer"),
                                                "Local Pack Description": r.get("Brand") or r.get("Format"),
                                                "Formulation": r.get("Format"), # approximation
                                                "Fill Size": fill_size_from_format(r.get("Format")),
                                                "Strength": None, # Hard to parse from unstructured text without specific logic
                                                "Strength Unit": None,
                                                "LOCAL_PACK_CODE": din_val.zfill(8),
                                            }
                                            writer.writerow(row)
                                            total_rows += 1
                                            page_rows += 1
                                        ...
                                        if page_rows > 0:
                                            logger.info(f"Page {page_no_1idx}: AI recovered {page_rows} rows.")
                            except Exception as e_hybrid:
                                logger.warning(f"Hybrid check failed for page {page_no_1idx}: {e_hybrid}")
                        # --------------------------------
    ...
    print(f"[OK] Saved: {str(OUTPUT_CSV)}")
    print(f"[OK] Log file: {str(LOG_FILE)}")
