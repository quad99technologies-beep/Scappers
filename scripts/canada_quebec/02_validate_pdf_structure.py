...
from config_loader import (
    ...
    PDF_STRUCTURE_REPORT_SUFFIX_JSON, PDF_STRUCTURE_REPORT_SUFFIX_TXT, PDF_STRUCTURE_FLAGS_SUFFIX_CSV,
    VALIDATION_INPUT_PDF_NAME,
    DB_ENABLED
)
...
INPUT_PDF = SPLIT_PDF_DIR / VALIDATION_INPUT_PDF_NAME
...
def write_reports(result: Dict[str, Any], *, stem: str = "pdf") -> Dict[str, Path]:
    ...
    json_path = OUTPUT_DIR / f"{safe_stem}{PDF_STRUCTURE_REPORT_SUFFIX_JSON}"
    txt_path = OUTPUT_DIR / f"{safe_stem}{PDF_STRUCTURE_REPORT_SUFFIX_TXT}"
    csv_path = OUTPUT_DIR / f"{safe_stem}{PDF_STRUCTURE_FLAGS_SUFFIX_CSV}"
...
def main() -> None:
    ...
    if DB_ENABLED:
        try:
            ...
            if run_id:
                # Log stats
                # Rows extracted = number of DIN pages? Or just 0 as it's validation.
                # Validation errors = flagged pages count?
                
                # Sum up stats if possible
                total_flagged = sum(len(res["flagged"]) for res in results_by_annex.values() if isinstance(res, dict) and "flagged" in res)
                
                db.log_step(run_id, "Validate PDF Structure", "COMPLETED", 0, 0.0, combined)
                print(f"Validation stats saved to DB for run {run_id}")
