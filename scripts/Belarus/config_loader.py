"""
Configuration Loader for Belarus Scraper (Platform Config Integration)

This module wraps platform_config.py for centralized path management.
All configuration is loaded from config/Belarus.env.json.

Precedence (highest to lowest):
1. Runtime overrides
2. Environment variables (OS-level)
3. Platform config (config/Belarus.env.json)
...
# Now: scripts/Belarus/config_loader.py -> parents[2] = repo root
...
def load_env_file():
    """
    Load environment variables from platform.env and Belarus.env.
...
def require_env(key: str) -> str:
    ...
    if value is None or value == "":
        raise ValueError(f"Required environment variable '{key}' is not set in config file. Please add it to config/Belarus.env.json")
    return value
...
def get_base_dir() -> Path:
    """
    Get base directory for Belarus scraper.
...
def get_input_dir(subpath: str = None) -> Path:
    """
    Get input directory - uses Documents/ScraperPlatform/input/Belarus/
...
def get_output_dir(subpath: str = None) -> Path:
    """
    Get output directory - uses Documents/ScraperPlatform/output/Belarus/
...
if __name__ == "__main__":
    print("=" * 60)
    print("Belarus Config Loader - Diagnostic")
    print("=" * 60)
    ...
    print(f"  SCRIPT_01_GENERIC_LIST_CSV: {getenv('SCRIPT_01_GENERIC_LIST_CSV', 'not set')}")
